...
### Если сборка падает с «requirements.txt not found»

Netlify клонирует только то, что есть в Git. Убедитесь, что вся папка `wiki-netlify` (включая `requirements.txt`, `netlify.toml`, `build.py`, `content/`, `static/`, `templates/`) закоммичена и запушена в ту ветку, которую собирает Netlify. Из корня репозитория:

```bash
git status
git add wiki-netlify/
git commit -m "Add wiki-netlify (ensure requirements.txt and all files are committed)"
git push
```

Папку `dist/` в репозиторий коммитить не нужно (её создаёт сборка).
